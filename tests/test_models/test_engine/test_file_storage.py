...
class TestFileStorage(unittest.TestCase):
    ...
    def test_instanceAttributes(self):
        ...
        self.assertEqual(
            self.storage._FileStorage__file_path.endswith('.json'), True)
    ...
    def test_allNotEmpty(self):
        ...
        self.storage._FileStorage__objects = self.objects_dict
        ...
        for key, obj in self.objects_dict.items():
            with self.subTest(key=key, obj=obj):
                ...
                self.assertIsInstance(
                    self.storage._FileStorage__objects[key], type(obj))

        self.assertEqual(self.storage._FileStorage__objects, self.objects_dict)

    def test_saveEmpty(self):
        """Test the save method when __objects is empty."""
        with mock.patch(
            'models.engine.file_storage.open', new=mock.mock_open()
        ) as fake_file:
            self.storage.save()
            fake_file.assert_called_once_with(
                self.storage._FileStorage__file_path, 'w', encoding='utf-8')
        ...
        self.json_file_contents = json.dumps(
            {
                ...
                "Place.c4663bbb-0918-4c0a-9f28-1aba33ce53ff": {
                    ...
                    "amenity_ids": ["60af1e59-4fde-4b8c-89e8-bd4bd4e4ee8e",
